...
from qtpy.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                           ...
                           QGraphicsTextItem, QGraphicsEllipseItem, QMenuBar, QMenu, QFileDialog, QMessageBox)
...
class DNAVisualizerFinal(QMainWindow):
        ...
        if file_name:
            try:
                with open(file_name, 'r', encoding='utf-8') as file:
                    ...
                    # فقط کاراکترهای معتبر DNA را نگه دار
                    valid_chars = {'A', 'T', 'C', 'G'}
    ...
    def init_dna_graphics(self):
        ...
        self.dna_view.setFixedHeight(350)  # افزایش ارتفاع برای نمایش بهتر
    ...
    def draw_dna_helix(self):
        ...
        center_y = 175  # مرکز عمودی جدید
        radius = 80  # افزایش شعاع برای فاصله بیشتر بین رشته‌ها
        base_pairs = len(self.sequence)
        
        # رسم دو رشته DNA به صورت مارپیچ
        for i in range(base_pairs):
            angle = i * (2 * math.pi / 10)
            x = 100 + i * 50  # افزایش فاصله افقی
            y1 = center_y - radius * math.sin(angle)
            y2 = center_y + radius * math.sin(angle)
            
            # نمایش بازها به صورت دایره‌ای روی رشته اول (بالا)
            nt = self.sequence[i]
            ...
            ellipse.setZValue(2)  # مقدار Z بالاتر برای نمایش در جلو
            self.dna_scene.addItem(ellipse)
            
            # نمایش حرف نوکلئوتید (بالا)
            text = QGraphicsTextItem(nt)
            text.setPos(x - 8, y1 - 10)
            text.setFont(QFont("Arial", 12, QFont.Bold))
            text.setDefaultTextColor(Qt.white)
            text.setZValue(3)  # بالاترین مقدار Z برای متن
            self.dna_scene.addItem(text)
            
            # نمایش مکمل روی رشته دوم (پایین) - اگر محاسبه شده
            if self.complement_sequence and i < len(self.complement_sequence):
                comp_nt = self.complement_sequence[i]
                ...
                comp_ellipse.setZValue(2)  # مقدار Z بالاتر
                ...
                comp_text.setZValue(3)  # بالاترین مقدار Z برای متن
                self.dna_scene.addItem(comp_text)
            
            # رسم اتصالات بین بازها (بعد از رسم دایره‌ها)
            if i > 0:
                prev_angle = (i-1) * (2 * math.pi / 10)
                prev_x = 100 + (i-1) * 50
                prev_y1 = center_y - radius * math.sin(prev_angle)
                prev_y2 = center_y + radius * math.sin(prev_angle)
                ...
                path_item1.setZValue(1)  # مقدار Z پایین‌تر از دایره‌ها
                ...
                connector_item.setZValue(0)  # پایین‌ترین مقدار Z
        
        # تنظیم اندازه صحنه
        new_width = max(1000, base_pairs * 50 + 200)
    ...
    def scroll_to_end(self):
        if self.sequence:
            scroll_pos = len(self.sequence) * 50 - self.dna_view.width() + 200
